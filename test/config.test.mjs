...
const ENV_SNAPSHOT = new Map(CONFIG_ENV_KEYS.map((key) => [key, process.env[key]]));
...
test('loadConfig uses defaults when no env/config/overrides are set', async () => {
  ...
  assert.equal(config.headerTemplatePath, path.resolve(cwd, './templates/header.html'));
  assert.equal(config.footerTemplatePath, path.resolve(cwd, './templates/footer.html'));
  assert.equal(config.documentTemplatePath, path.resolve(cwd, './templates/document.html'));
  assert.equal(config.bookLayoutCssPath, path.resolve(cwd, './styles/pdf-book-layout.css'));
...
test('CLI overrides take highest precedence over config and env', async () => {
  ...
  await writeFile(path.join(cwd, '.pipeline.env'), ['PDF_PRINT_READY=false', 'PDF_BLEED=2mm'].join('\n'), 'utf8');
  await writeFile(path.join(cwd, '.pipeline.config.json'), JSON.stringify({ pdfPrintReady: false, pdfBleed: '6mm' }), 'utf8');

  const config = await loadConfig(cwd, { printReady: true, pdfBleed: '10px', headerTemplatePath: './custom-header.html' });

  assert.equal(config.pdfPrintReady, true);
  assert.equal(config.pdfBleed, '10px');
  assert.equal(config.headerTemplatePath, path.resolve(cwd, './custom-header.html'));
...
test('invalid booleans are rejected with clear errors', async () => {
  ...
  await assert.rejects(() => loadConfig(cwd, { printReady: 'not-a-boolean' }), /PDF_PRINT_READY must be either "true" or "false"/);
  await assert.rejects(() => loadConfig(cwd, { sshUploadEnabled: 'not-a-boolean' }), /SSH_UPLOAD_ENABLED must be either "true" or "false"/);
  await assert.rejects(() => loadConfig(cwd, { remarkableUploadEnabled: 'not-a-boolean' }), /REMARKABLE_UPLOAD_ENABLED must be either "true" or "false"/);
});
...
test('invalid ssh port and invalid bleed are rejected', async () => {
  ...
  await assert.rejects(() => loadConfig(cwd, { sshPort: 'abc' }), /SSH_PORT must be a valid integer/);
  await assert.rejects(() => loadConfig(cwd, { pdfBleed: '3' }), /PDF_BLEED must be a CSS length/);
});
...
test('cli --print-config=json shows evaluated values, source map and file state', async () => {
  ...
  await writeFile(path.join(cwd, '.pipeline.env'), ['OUTPUT_DIR=./env-out', 'PDF_BLEED=8mm'].join('\n'), 'utf8');
  await writeFile(path.join(cwd, '.pipeline.config.json'), JSON.stringify({ outputDir: './config-out' }), 'utf8');

  const result = await runCli(['build', '--print-config=json', '--bleed', '12px'], cwd);
...
test('cli --print-config table output contains diagnostics and source rows', async () => {
  ...
  await writeFile(path.join(cwd, '.pipeline.env'), 'OUTPUT_DIR=./env-out\n', 'utf8');
