...
export function runScpCopy(host, localPath, remotePath, { port } = {}) {
  ...
  return spawnSync('scp', [...portArgs, localPath, `${host}:${remotePath}`], { stdio: 'inherit' });
}

export function runRsyncCopy(args, { sshPort } = {}) {
  const sshCommand = sshPort ? `ssh -p ${sshPort}` : 'ssh';
  return spawnSync('rsync', ['-avz', '-e', sshCommand, ...args], { stdio: 'inherit' });
}
...
export function sshAvailable(host, { port } = {}) {
  ...
  const result = spawnSync(
    'ssh',
    ['-o', 'BatchMode=yes', '-o', 'ConnectTimeout=3', ...portArgs, host, 'exit'],
    { stdio: 'ignore' },
  );
